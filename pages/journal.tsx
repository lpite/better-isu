...
function Day({ date, type, grade }: DayProps) {
  ...
  return (
    <div
      className={`min-w-28 shrink-0 h-20 rounded flex flex-col py-1 px-1.5 dark:bg-gray-800 bg-slate-200`}
    >
      <div className="grow flex items-start justify-start">
        <span className="text-2xl dark:text-white grow">{date}</span>
        <span
          className={`text-3xl font-bold leading-none ${colors[isNaN(Number(grade)) ? grade : Math.floor(Number(grade))]}`}
        >
          {grade}
        </span>
      </div>
      <span className="dark:text-white leading-tight">
