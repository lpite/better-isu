...
export default async function getRatingPage(session: Session) {
  const firstPage = await fetchAndDecode(
    "https://isu1.khmnu.edu.ua/isu/dbsupport/students/rating.php",
    {
      headers: {
  ...
  const semsetersPage = await fetchAndDecode(
    "https://isu1.khmnu.edu.ua/isu/dbsupport/students/rating.php",
    {
      method: "POST",
      headers: {
  ...
  const linksArray = semsetersPage.html
  ...
  const lastSemesterLink = linksArray[linksArray.length - 2]
    ...
    ?.split("'")[1]
    .split("'")[0];
  const paramStr = semsetersPage.html
    .querySelector("[name=ParamStr]")
    ?.getAttribute("value");
  const keyStr = semsetersPage.html
  ...
  const ratingPage = await fetchAndDecode(
    "https://isu1.khmnu.edu.ua/isu/dbsupport/students/rating.php",
    {
      method: "POST",
      headers: {
  ...
  const ratingTableRows = ratingPage.html
